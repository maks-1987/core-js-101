...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const arr = [a, b];
  arr.sort((x, y) => x - y);
  const leftBr = isStartIncluded ? '[' : '(';
  const rightBr = isEndIncluded ? ']' : ')';
  return `${leftBr}${arr.join(', ')}${rightBr}`;
}
...
function getDigitalRoot(num) {
  // throw new Error('Not implemented');
  const sum = num.toString().split('').reduce((acc, next) => Number(acc) + Number(next), 0);
